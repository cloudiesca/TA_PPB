...
// src/utils/cartStorage.js - PERMANENT FIX
const CART_KEY = 'wearify_cart';
...
export const addToCart = (product, quantity = 1, size = null, color = null) => {
    try {
        ...
        // CRITICAL FIX: Ensure proper typing and minimum quantity
        const productPrice = Number(product.price) || 0;
        const productQuantity = Math.max(1, parseInt(quantity, 10) || 1); // Minimum 1
        const productStock = parseInt(product.stock, 10) || 999;
        const productImage = product.image_url || product.image || 'https://via.placeholder.com/400?text=No+Image';

        console.log('Adding to cart:', product.name, 'Qty:', productQuantity);
        ...
        if (existingItemIndex > -1) {
            const newQuantity = cart[existingItemIndex].quantity + productQuantity;
            cart[existingItemIndex].quantity = Math.min(newQuantity, productStock);
        } else {
            const newItem = {
                id: product.id,
                name: product.name || 'Unknown Product',
                ...
                category: product.categories?.name || product.category || 'Category',
                stock: productStock,
                quantity: Math.min(productQuantity, productStock), // Ensure minimum 1
                size: size,
                color: color,
                gender: product.gender || 'unisex'
            };
...
export const updateCartItemQuantity = (productId, size, color, quantity) => {
    try {
        ...
        const newQuantity = Math.max(1, parseInt(quantity, 10) || 1); // Minimum 1

        if (newQuantity <= 0) {
            return removeFromCart(productId, size, color);
        }

        const maxStock = parseInt(cart[itemIndex].stock, 10) || 999;
